...
if (is_post()) {
    ...
    if(!$email||!$pass){
        ...
        // SEF rolünü de giriş yapabilir roller listesine ekledik.
        if(login_user_demo_or_db($email,$pass,['GENEL','MUDUR','SEF'])){
            flash_set('success','Giriş başarılı.');
            $ret=$_GET['return']??'';
            redirect($ret?app_url(ltrim($ret,'/')):app_url('yonetim/pano.php'));
        }else{
            flash_set('error','Geçersiz bilgi.');
        }
    }
}
?>
...
<div class="login-box">
    ...
    <form method="post">
        ...
        <div class="small text-muted mt-2">
            (Demo Admin) <?php echo h(env('ADMIN_DEMO_EMAIL')); ?> / <?php echo h(env('ADMIN_DEMO_PASS')); ?>
        </div>
    </form>
</div>
</body>
